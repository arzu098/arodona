    ...
    <div className="min-h-screen bg-[#fceec6]">
      ...
      <header className="bg-[#3E2F2A] text-white px-4 sm:px-6 lg:px-[5%] py-4 sm:py-5">
        <div className="max-w-[1400px] mx-auto flex justify-between items-center">
          ...
          <nav className="hidden md:flex gap-6 lg:gap-11 uppercase text-[11px] lg:text-[13px] tracking-wider font-light">
            <Link to="/rings" className="hover:text-yellow-500 transition-colors border-b-2 border-yellow-500 pb-1">RINGS</Link>
        ...
        {isMobileMenuOpen && (
          <div className="md:hidden bg-[#2d2219] mt-4 py-4 px-4 rounded">
            <nav className="flex flex-col gap-3">
              <Link to="/rings" className="text-white hover:text-yellow-500 transition-colors border-b-2 border-yellow-500 pb-2 uppercase text-sm tracking-wider">RINGS</Link>
      ...
      <section className="px-4 sm:px-6 lg:px-[5%] py-8 sm:py-10 lg:py-12 text-center bg-[#fceec6]">
        <div className="max-w-[700px] mx-auto">
          <h1 className="text-3xl sm:text-4xl lg:text-5xl font-serif font-normal text-gray-900 mb-2 sm:mb-3">Rings</h1>
          <p className="text-xs sm:text-sm text-gray-800 leading-relaxed px-4">
            Where artistry meets emotion — each ring is a testament to exquisite craftsmanship and eternal beauty.
          </p>
        </div>
      </section>
      ...
      <section className="px-4 sm:px-6 lg:px-[5%] py-8 sm:py-10 lg:py-12 bg-[#fceec6]">
        <div className="max-w-[1400px] mx-auto grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 sm:gap-5">
          {isLoading ? (
            ...
            <div className="col-span-full text-center py-12">
              <p className="text-gray-600">No rings available at the moment</p>
            </div>
          ) : (
            rings.map((ring) => (
            <div key={ring.id} className="bg-white rounded-sm overflow-hidden hover:shadow-lg transition-all relative group">
              <Link to={`/product/${ring.id}`} className="block cursor-pointer">
                <div className="w-full h-[280px] flex items-center justify-center bg-white p-8">
                  <img src={ring.image} alt={ring.name} className="max-w-full max-h-full object-contain" />
                </div>
                <div className="px-4 py-3 text-center bg-white">
                  <h3 className="text-xs uppercase tracking-wider text-gray-800 mb-1.5 font-medium">{ring.name}</h3>
                  {ring.brand && (
                    <p className="text-xs text-gray-500 mb-1">Brand: {ring.brand}</p>
                  )}
                  <p className="text-sm font-semibold text-gray-900">{ring.price}</p>
                </div>
              </Link>
              <button 
                onClick={async (e) => {
                  ...
                  const wishlistProduct = {
                    id: ring.id,
                    name: ring.name,
                    price: ring._rawPrice || parseFloat(ring.price.replace(/[₹$,]/g, '')),
                    image: ring.image
                  };
                  if (isInWishlist(ring.id)) {
                    const result = await removeFromWishlist(ring.id);
                    if (!result.success) {
              ...
              {isInCart(ring.id) ? (
                <div className="w-full flex items-center justify-center gap-2 bg-green-600 hover:bg-green-700 text-white py-2.5 rounded transition-all">
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
                      const item = cartItems.find(i => i.id === ring.id || i.product_id === ring.id);
