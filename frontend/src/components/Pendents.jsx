...
const Pendents = () => {
  ...
  useEffect(() => {
    const fetchPendents = async () => {
      try {
        setIsLoading(true);
        const response = await productService.getProductsByCategory('pendant', { limit: 50 });
        const products = response.products || response.items || [];
        setPendents(products.map(p => ({
          id: p._id || p.id,
          name: p.name,
          price: `₹ ${p.price?.toLocaleString('en-IN')}`,
          image: p.images?.[0]?.url || '/Images/default.png',
          _rawPrice: p.price
        })));
      } catch (error) {
        console.error('Error fetching pendents:', error);
        setPendents([]);
      } finally {
        setIsLoading(false);
      }
    ...
    <div className="min-h-screen bg-[#fceec6]">
      ...
      <header className="bg-[#3E2F2A] text-white px-4 sm:px-6 lg:px-[5%] py-4 sm:py-5">
        <div className="max-w-[1400px] mx-auto flex justify-between items-center">
          ...
          <nav className="hidden md:flex gap-6 lg:gap-11 uppercase text-[11px] lg:text-[13px] tracking-wider font-light">
            ...
            <Link to="/pendents" className="hover:text-yellow-500 transition-colors border-b-2 border-yellow-500 pb-1">PENDENTS</Link>
        ...
        {isMobileMenuOpen && (
          <div className="md:hidden bg-[#2d2219] mt-4 py-4 px-4 rounded">
            <nav className="flex flex-col gap-3">
              ...
              <Link to="/pendents" className="text-white hover:text-yellow-500 transition-colors border-b-2 border-yellow-500 pb-2 uppercase text-sm tracking-wider">PENDENTS</Link>
              <Link to="/necklaces" className="text-white hover:text-yellow-500 transition-colors pb-2 uppercase text-sm tracking-wider">NECKLACES</Link>
            </nav>
          </div>
        )}
      </header>
      ...
      <section className="px-4 sm:px-6 lg:px-[5%] py-8 sm:py-10 lg:py-12 text-center bg-[#fceec6]">
        <div className="max-w-[700px] mx-auto">
          <h1 className="text-3xl sm:text-4xl lg:text-5xl font-serif font-normal text-gray-900 mb-2 sm:mb-3">Pendents</h1>
          <p className="text-xs sm:text-sm text-gray-800 leading-relaxed px-4 sm:px-0">
            Personal treasures close to your heart — our pendents are designed to tell your unique story with elegance and grace.
          </p>
        </div>
      </section>
      ...
      <section className="px-4 sm:px-6 lg:px-[5%] py-8 sm:py-10 lg:py-12 bg-[#fceec6]">
        <div className="max-w-[1400px] mx-auto grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 sm:gap-5">
          {isLoading ? (
            <div className="col-span-full text-center py-12"><p className="text-gray-600">Loading pendents...</p></div>
          ) : pendents.length === 0 ? (
            <div className="col-span-full text-center py-12"><p className="text-gray-600">No pendents available at the moment</p></div>
          ) : (
            pendents.map((pendent) => (
            <div key={pendent.id} className="bg-white rounded-sm overflow-hidden hover:shadow-lg transition-all relative group">
              <Link to={`/product/${pendent.id}`} className="block cursor-pointer">
                <div className="w-full h-[280px] flex items-center justify-center bg-white p-8">
                  <img src={pendent.image} alt={pendent.name} className="max-w-full max-h-full object-contain" />
                </div>
                <div className="px-4 py-3 text-center bg-white">
                  <h3 className="text-xs uppercase tracking-wider text-gray-800 mb-1.5 font-medium">{pendent.name}</h3>
                  {pendent.brand && (
                    <p className="text-xs text-gray-500 mb-1">Brand: {pendent.brand}</p>
                  )}
                  <p className="text-sm font-semibold text-gray-900">{pendent.price}</p>
                </div>
              </Link>
              <button 
                onClick={async (e) => {
                  e.stopPropagation();
                  if (isInWishlist(pendent.id)) {
                    await removeFromWishlist(pendent.id);
                  } else {
                    const result = await addToWishlist({ id: pendent.id, name: pendent.name, price: pendent._rawPrice || parseFloat(pendent.price.replace(/[₹$,]/g, '')), image: pendent.image });
                ...
                className={`absolute top-3 left-3 border w-8 h-8 rounded-sm text-lg cursor-pointer flex items-center justify-center transition-all ${
                  isInWishlist(pendent.id) ? 'bg-red-50 border-red-300 text-red-600 hover:bg-red-100' : 'bg-white border-gray-300 hover:bg-gray-100'
                }`}
              >
                {isInWishlist(pendent.id) ? '♥' : '♡'}
              </button>
              {isInCart(pendent.id) ? (
                <div className="w-full flex items-center justify-center gap-2 bg-green-600 hover:bg-green-700 text-white py-2.5 rounded transition-all">
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
                      const item = cartItems.find(i => i.id === pendent.id || i.product_id === pendent.id);
                ...
                <button 
                  onClick={async (e) => {
                    e.stopPropagation();
                    await addToCart({ id: pendent.id, name: pendent.name, price: pendent._rawPrice || parseFloat(pendent.price.replace(/[₹$,]/g, '')), image: pendent.image, quantity: 1 });
                  }}
